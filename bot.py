...
WEBHOOK_URL = os.environ["WEBHOOK_URL"]  # örn: https://beklentilerbot.onrender.com
...
async def safe_reply(message, text, reply_markup=None):
    """Flood control (RetryAfter) yakalar, bot crash olmaz."""
    try:
...
async def entry_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if et == "survey":
        keyboard = [[InlineKeyboardButton(x, callback_data=f"survey_{x}")] for x in SURVEYS]
...
async def source(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # survey listeden seçilirse callback_query gelir
    ...
    await safe_reply(msg, "Tahmin türünü seç:", InlineKeyboardMarkup(keyboard))
    return FTYPE
...
async def period(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not p:
        await safe_reply(update.message, "Geçersiz dönem. Örnek: 2026-04")
        return PERIOD
...
def build_app():
    ...
    conv = ConversationHandler(
        ...
        fallbacks=[],
        per_message=True,  # warning'i azaltır
    )
